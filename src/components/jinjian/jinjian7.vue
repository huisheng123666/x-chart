<template>
  <div class="home-1">
    <block-title title="各银行金融服务（贷款余额）">
      <span>单位：万元</span>
    </block-title>
    <ul class="t-head">
      <li>银行名称</li>
    ...
    <TransitionGroup :name="slideName" tag="div" class="list">
      <div class="h1-item" v-for="item in list" :key="item.id">
        ...
        <div class="col">{{ formatNum(item.gs_amount) }}</div>
        <div class="col">{{ formatNum(item.zd_amount) }}</div>
        <div class="col">{{ formatNum(item.zjtx_amount) }}</div>
        <div class="col">{{ formatNum(item.nycyh_amount) }}</div>
        <div class="col">{{ formatNum(item.ldzd_amount) }}</div>
        <div class="col">{{ formatNum(item.byqy_amount) }}</div>
      </div>
    </TransitionGroup>
  </div>
</template>
...
.home-1
  ...
  .t-head
    display grid
    grid-template-columns 1fr repeat(6, 62px)
    height 60px
    align-items center
