...
const XArr = ["农业银行", "农商行", "汉口银行", "招商银行", "光大银行"];
const XArr1 = ["网捷贷", "云e贷", "闪电贷", "市民贷", "随心贷"];
// 第一条数据
let data1 = ref(new Array(12).fill(1).map(i => (Math.random()).toFixed(2)));

setInterval(() => {
  data1.value = new Array(12).fill(1).map(i => (Math.random()).toFixed(2))
}, 3000)
// 蓝色
const colorsPlan = [
  {
    ...
    series: [
      // 第一条数据进度柱子
      {
        ...
        label: {
          ...
          lineHeight: 20,
          formatter: (params: any) => {
            const percentInt = params.data.toString().split('.')[1]
            return `${XArr1[params.dataIndex]}\n${Number(percentInt) + '%'}`
          }
        }
      },
      // 第一条数据上面正方形：颜色
